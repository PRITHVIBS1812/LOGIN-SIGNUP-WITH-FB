...
class mechBlock extends StatefulWidget {
  const mechBlock({Key? key}) : super(key: key);

  @override
  State<mechBlock> createState() => _mechBlockState();
}

class _mechBlockState extends State<mechBlock> {
  @override
  Query dbRef = FirebaseDatabase.instance.ref().child('MECHCLASS');
        ...
        child: Column(
          ...
          children: [
            if(request['CLASS-TYPE']=='Classroom')
              mechclass(request['CLASS-NAME'], request['STRENGTH'], request['SPEAKER-PROJECTOR'], request['AC']),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text("MECH BLOCK",style: TextStyle(fontFamily: 'Arimo',color: Colors.white,fontSize:15,fontWeight: FontWeight.bold),),
        centerTitle: true,
  ...
  Material mechclass(String classnum,String strength,String speakerAndprojector,String ac) {
    return Material(
      elevation: 4,
      color: Colors.white,
